...
### Bosqich 11 — Kompyuter yoqilganda ilovani avtomatik ishga tushirish (Login Item, macOS)

- **Muammo:** Bosqich 10 ilovani X bilan yopilganda fonda ishlab turishini ta'minlaydi, lekin bu faqat "ilova allaqachon ishga tushgan" holat uchun. Kompyuter butunlay qayta yoqilsa (elektr uzilishi, OS yangilanishi va h.k.), process ham yo'qoladi — kimdir uni qo'lda qayta ochmaguncha server ishlamaydi, veb-sayt chek chiqara olmaydi.
- **Yechim:** ilova birinchi marta ishga tushganda o'zini macOS'ning "Login Items" (kompyuterga kirilganda avtomatik ochiladigan ilovalar) ro'yxatiga o'zi qo'shib qo'yadi — xuddi Dropbox/Slack kabi. Foydalanuvchi buni qo'lda sozlamaydi.
- **Muhim (foydalanuvchi bilan muhokama qilingan, 2026-09-12):** bu — "har doim majburan ochiq turish" degani emas. Faqat "kompyuter yoqilganda/login qilinganda bir marta ishga tush" degani. Agar foydalanuvchi o'sha kuni ishlamoqchi bo'lmasa, tray menyusidan "Chiqish"ni bosib, ilovani to'liq yopa oladi — u kun davomida qayta o'zi ochilmaydi, faqat keyingi safar kompyuter qayta yoqilganda yana avtomatik ishga tushadi.
- **Texnik yechim (macOS):** uchinchi tomon paketi (`launch_at_startup`) sandboxed macOS ilovalar uchun qo'shimcha `LaunchAtLogin` Swift Package + Xcode Build Phase talab qiladi (murakkab, Xcode GUI orqali SPM bog'lanishi kerak). Buning o'rniga to'g'ridan-to'g'ri Apple'ning o'z native API'si — `ServiceManagement` freymvorkidagi `SMAppService.mainApp.register()` — ishlatiladi (`macos/Runner/AppDelegate.swift`da, `applicationDidFinishLaunching`da). Hech qanday tashqi paket, Xcode sozlash yoki entitlements o'zgarishi shart emas.
...
### Bosqich 14 — PDF/PDFium ham xira chiqdi: Windows uchun to'g'ridan-to'g'ri GDI+ (2026-09-16)

- **Muammo:** Bosqich 12'dagi Windows uchun PDF+drayver yo'li (`Printing.directPrintPdf`) amalda sinalganda, chek matni **avvalgi RAW ESC/POS'dan ham xiraroq** chiqdi — arxitekturani o'zgartirishning asosiy sababi aynan tiniqlik bo'lgani uchun bu qabul qilinmaydigan natija edi.
- **Ildiz sabab topildi (`printing` paketi, v5.14.3, `windows/print_job.cpp`):** Windows tomonida bu paket PDF'ni PDFium (`FPDF_RenderPage`) orqali printer DC'ga chizadi. PDFium matnni (ayniqsa bizning holatda — `pdf` paketi ichiga o'rnatilgan, subset qilingan `PdfGoogleFonts.roboto*()` shrifti bilan) GDI vektor buyruqlari sifatida emas, balki **oldindan anti-aliased (kulrang qirrali) rastr** sifatida chizadi; buni printer drayveri o'zining dithering/halftone algoritmi bilan qayta piksellashtirganda, nozik Roboto chiziqlari "erib" ketadi.
- **Solishtirish — C# ilova nega tiniq (`~/Desktop/PrinterTestApp — Latest/MainForm.cs`):** maxsus "anti-aliasingsiz" sozlama YO'Q (`RenderReceipt`da hech qanday `TextRenderingHint` o'rnatilmagan) — sabab shunchaki shundaki, u PDF orqali emas, `System.Drawing`ning GDI+ API'si (`Graphics.DrawString`/`DrawLine`/`FillRectangle`) orqali **to'g'ridan-to'g'ri** printer DC'ga chizadi, tizim shrifti (Arial) bilan — PDF/PDFium bosqichi butunlay yo'q.
- **Qaror (foydalanuvchi, 2026-09-16):** Windows uchun `pdf`/`PdfGoogleFonts`/`Printing.directPrintPdf` yo'li butunlay olib tashlandi, o'rniga C#ning aynan o'zi ishlatgan yo'l — **native GDI+ orqali to'g'ridan-to'g'ri printer drayveriga chizish** — qo'shildi. Chek ko'rinishi (shrift o'lchamlari, bo'shliqlar, joylashuv) o'zgarmasligi kerak edi (foydalanuvchi talabi) — faqat rasterlash bosqichi almashtirildi.
- **Amalga oshirilgan:**
  - `windows/runner/gdi_receipt_printer.h/.cpp` (yangi) — `PrintReceiptBlocks()`: printer DC'ni ochadi (`CreateDC`, `dm=nullptr` — C#dagi kabi drayverning o'z standart sozlamalari, masalan 80mm rulon, ishlatiladi), `Gdiplus::Graphics`ni `UnitPoint`da (PDF'dagi bilan bir xil pt birligi) sozlaydi, va Dart'dan kelgan tayyor "bloklar" ro'yxatini (`text`/`row`/`headerRich`/`statusBox`/`divider`/`image`/`barcodeBars`/`socialRow`/`spacer`) yuqoridan pastga chizadi — GDI+ da hech qanday maxsus anti-aliasing/hint sozlamasi qo'yilmaydi (C#dagi standart holatga mos).
  - `lib/service/receipt/receipt_gdi_blocks.dart` (yangi, o'chirilgan `receipt_pdf_builder.dart`ning o'rnini bosadi) — uslub qoidalarining o'zi (shrift o'lchamlari 12/16px→pt, bo'shliqlar, compact rejim, status ramkasi va h.k.) **aynan bir xil qoladi**, faqat `pw.Widget` o'rniga oddiy `Map` "blok"lar quriladi. Barcode uchun `barcode` paketi to'g'ridan-to'g'ri (`Barcode.code128().make()`) ishlatiladi (avval `pdf` orqali bilvosita edi). Ijtimoiy tarmoq SVG ikonkalari (GDI+ SVG chiza olmagani uchun) kichik PNG rastrga oldindan aylantiriladi (`flutter_svg` + `dart:ui`).
  - `lib/service/printer/gdi_receipt_printer.dart` (yangi) — `MethodChannel('miraprint/gdi_print')` orqali native tomonni chaqiradi.
  - `printer_section.dart` va `print_route.dart`dagi Windows tarmoqlari (`Printing.directPrintPdf` + `buildReceiptPdf`) yangi `printReceiptViaGdi()`ga almashtirildi; kesish komandasi (`sendWindowsCutCommand`) o'zgarishsiz, chop etishdan keyin xuddi avvalgidek yuboriladi.
  - `pdf` paketi `pubspec.yaml`dan olib tashlandi (endi faqat `printing`ning o'zi ichida transitive bog'liqlik sifatida qoladi — `printing.listPrinters()`/printer ro'yxati hamon shu paket orqali).
- **Hali tekshirilmagan:** bu ham (Bosqich 12'dagi kabi) faqat **haqiqiy Windows kompyuter + haqiqiy o'rnatilgan printer drayveri** bilan sinalganda tasdiqlanadi — native C++ kodni macOS'da compile qilib bo'lmaydi (faqat Dart tomoni `flutter analyze` bilan tekshirildi, xatosiz).
